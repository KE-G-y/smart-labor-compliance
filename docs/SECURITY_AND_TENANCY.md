...
| tenant_admin | 本租户账号、来源、日志、反馈、知识包、向量知识库版本 |
| operator | 本租户来源、日志、反馈、知识包、向量知识库版本 |
| viewer | 本租户概览与日志只读 |
...
| 字段 | 含义 | 当前配置说明 | 调优建议 |
| --- | --- | --- | --- |
| `pool_size=10` | SQLAlchemy 连接池中长期保留的基础连接数。每个后端进程会维护自己的连接池。 | 单个 worker 最多常驻 10 条 MySQL 连接，适合中小并发的后台管理、问答日志、来源目录和知识包查询等场景。 | 如果数据库连接数紧张可降低到 5；如果接口并发稳定较高且 MySQL 资源充足，可适当提高。 |
